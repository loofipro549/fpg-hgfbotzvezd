...
async def ask_cast_question(message, state, cast_selected=None):
    kb = InlineKeyboardMarkup()
    casts = ["OSINT", "Эдитор", "Сносер", "Соц инженер", "Кодер"]
    for c in casts:
        kb.add(InlineKeyboardButton(c, callback_data=f"cast_{c}"))
    await message.answer("Выберите вашу вторую касту если есть(Если нету нажмите на ту же касту):", reply_markup=kb)
    await state.set_state(Form.cast_choice)
    if cast_selected:
        await state.update_data(selected_casts=cast_selected)
    else:
        await state.update_data(selected_casts=[])
...
async def process_cast(call: types.CallbackQuery, state: FSMContext):
    ...
    # Спрашиваем "Вы только OSINT?" для OSINT или аналогично для других каст
    if cast == "OSINT":
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("Да", callback_data=f"only_yes_{cast}"))
        kb.add(InlineKeyboardButton("Нет", callback_data=f"only_no_{cast}"))
        await call.message.answer(f"Вы только {cast}?", reply_markup=kb)
    else:
        # Если не OSINT, сразу продолжаем вопросы
        await ask_question(call.message, state, 2)  # следующий вопрос после выбора роли
